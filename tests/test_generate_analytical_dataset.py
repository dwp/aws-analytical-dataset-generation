...
from steps.generate_analytical_dataset import (
    sanitize,
    validate,
    retrieve_id,
    retrieve_last_modified_date_time,
    retrieve_date_time_element,
    replace_element_value_wit_key_value_pair,
    wrap_dates,
    get_valid_parsed_date_time,
...
def test_sanitisation_processor_removes_desired_chars_in_collections():
    input_collection = (
        """{"fieldA":"a$\u0000","_archivedDateTime":"b","_archived":"c"}"""
    )
...
def test_if_decrypted_dbObject_is_a_valid_json():
    ...
    decrypted_object = {
        ...
        "type": "addressDeclaration",
        "contractId": 1234,
        "addressNumber": {"type": "AddressLine", "cryptoId": 1234},
        "addressLine2": None,
        "townCity": {"type": "AddressLine", "cryptoId": 1234},
        "postcode": "SM5 2LE",
        "processId": 1234,
        "effectiveDate": {
            "type": "SPECIFIC_EFFECTIVE_DATE",
...
def test_replace_value_when_current_value_is_string():
    ...
    actual = replace_element_value_wit_key_value_pair(
        old_json, "_lastModifiedDateTime", "$date", new_date
    )
...
def test_create_last_modified_if_missing_dates():
    ...
    oldJson = {"_id": {"test_key_a": "test_value_a", "test_key_b": "test_value_b"}}
...
def test_main(monkeypatch):
    generate_analytical_dataset.get_plaintext_key_calling_dks = (
        mock_get_plaintext_key_calling_dks
    )
    monkeypatch.setattr(
        generate_analytical_dataset, "get_published_db_name", mock_get_published_db_name
    )
    monkeypatch.setattr(
        generate_analytical_dataset, "get_staging_db_name", mock_get_staging_db_name
    )
    monkeypatch.setattr(generate_analytical_dataset, "get_tables", mock_get_tables)
    monkeypatch.setattr(
        generate_analytical_dataset, "get_spark_session", mock_get_spark_session
    )
    monkeypatch.setattr(
        generate_analytical_dataset,
        "get_dataframe_from_staging",
        mock_get_dataframe_from_staging,
    )
    monkeypatch.setattr(
        generate_analytical_dataset,
        "get_plaintext_key_calling_dks",
        mock_get_plaintext_key_calling_dks,
    )
    monkeypatch.setattr(generate_analytical_dataset, "persist_json", mock_persist_json)
    monkeypatch.setattr(
        generate_analytical_dataset,
        "create_hive_on_published",
        mock_create_hive_on_published,
    )
    monkeypatch.setattr(
        generate_analytical_dataset, "retrieve_secrets", mock_retrieve_secrets
    )
    monkeypatch.setattr(
        generate_analytical_dataset, "get_collections", mock_get_collections
    )
...
def mock_create_hive_on_published(
    parquet_location, published_database_name, spark, table_to_process
):
    return ""
