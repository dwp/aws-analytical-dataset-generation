...
cat << EOF | curl --data-binary @- "http://${adg_pushgateway_hostname}:9091/metrics/job/mark_test"
            adg_status{snapshot_type="$SNAPSHOT_TYPE", export_date="$EXPORT_DATE", cluster_id="$CLUSTER_ID"} $1
EOF
