...
data "template_file" "emr_setup_sh" {
  ...
  vars = {
    ...
    full_proxy         = local.internet_proxy["http_address"]
    ...
    truststore_aliases = "ca_cert"
    truststore_certs   = "s3://${local.env_certificate_bucket}/ca_certificates/dataworks/ca.pem,s3://dw-management-dev-public-certificates/ca_certificates/dataworks/ca.pem"
