...
def main():
	spark = SparkSession.builder.master("yarn").config("spark.sql.parquet.binaryAsString", "true").appName("spike").enableHiveSupport().getOrCreate()
  ...
  #)
	...
	datadf.show()
	datadf.write.parquet("S3_BUCKET/xxx.parquet")
	spark.sql("""CREATE EXTERNAL TABLE hive_spark_demo1(val STRING) STORED AS PARQUET LOCATION "S3_BUCKET/source_table_parquet/xxxxx.parquet" """)
...
def decrypt(cek, kek, iv, ciphertext):
	...
	params = {'keyId': kek}
	result = requests.post(url, params=params, data=cek, cert=('/opt/emr/analytical.pem','/opt/emr/analytical.key'), verify=False)
	content = result.json()
