...
This error occurs while doing GetObject operation in the ADG's Spark code. This occurs when there is time difference between local time and the internet time which is used by S3 while making boto3 requests.
...
On call with AWS support team it was discussed that troubleshooting this issue as it occurs randomly .
They have requested  to have complete error trace when happens next time. AWS EMR uses  chronyd daemon for Time Sync Service and possibly this error occurs when chronyd service stops.

In order to get the complete error trace following steps are suggested.
...
Ticket DW-6406 was created for this work.
